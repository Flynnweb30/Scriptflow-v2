                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px' }}>
                    {isEditing ? (
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                    {CONFIG.TAG_OPTIONS.map(tag => {
                                        ...
                                        return <button key={tag.id} type="button" onClick={() => {
                                            const current = Array.isArray(formData.tags) ? formData.tags : [];
                                            setFormData({ ...formData, tags: checked ? current.filter(t => t !== tag.id) : [...current, tag.id] });
                                        }} style={{ border: `1px solid ${checked ? tag.color : '#334155'}`, background: checked ? `${tag.color}22` : '#0d1527', color: checked ? tag.color : '#94a3b8', borderRadius: 999, padding: '5px 9px', fontSize: 10, fontWeight: 800, cursor: 'pointer' }}>{tag.name}</button>;
                                    })}
                                </div>
                            </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            {Array.isArray(formData.tags) && formData.tags.length > 0 && <div>
                                <div style={{ fontSize: '12px', fontWeight: 800, color: '#94a3b8', marginBottom: '8px' }}>STATUS TAGS</div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{formData.tags.map(tagId => { const tag = Utils.getTagDefinition(tagId); return <span key={tagId} style={{ padding: '5px 9px', borderRadius: 999, background: `${tag.color}22`, color: tag.color, border: `1px solid ${tag.color}55`, fontSize: 10, fontWeight: 800 }}>{tag.name}</span>; })}</div>
                            </div>}
