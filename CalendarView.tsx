...
export const CalendarView: React.FC<CalendarViewProps> = ({
    ...
    const [openActivityMenu, setOpenActivityMenu] = useState<'meeting' | 'callback' | 'followup' | null>(null);
    const [meetingStageFilter, setMeetingStageFilter] = useState<'all' | 'initial' | 'followup'>('all');
    ...
    const getActivityColor = useCallback((appt: Appointment) => {
        ...
        if (['completed', 'held'].includes(status) || (appt.callbackCompleted === true)) return '#22c55e';
    ...
    // A meeting with a callback reminder is represented by one canonical appointment
    // plus a derived callback activity. This avoids duplicate Firestore records while
    // allowing the callback to appear independently in List/Calendar until completed.
    const activityAppointments = useMemo(() => {
        const derived: Appointment[] = [];
        appointments.forEach((appt) => {
            if (!appt.callbackSetting || appt.callbackSetting === 'none') return;
            const callbackTime = TimezoneUtils.calculateCallbackTime(appt);
            if (!callbackTime) return;
            const zone = (US_TIMEZONE_OPTIONS.find(o => o.value === normalizeUSTimezone(appt.timezone))?.iana) || 'America/Chicago';
            const parts = new Intl.DateTimeFormat('en-US', { timeZone: zone, year: 'numeric', month: '2-digit', day: '2-digit', hour: 'numeric', minute: '2-digit', hour12: true }).formatToParts(callbackTime);
            const get = (type: string) => parts.find(part => part.type === type)?.value || '';
            const callbackDate = `${get('year')}-${get('month').padStart(2,'0')}-${get('day').padStart(2,'0')}`;
            const callbackTimeLabel = `${get('hour')}:${get('minute').padStart(2,'0')} ${get('dayPeriod')}`;
            derived.push({
                ...appt,
                id: `${appt.id}__callback`,
                callbackSourceId: appt.id,
                appointmentType: 'callback',
                eventType: 'callback',
                callbackKind: appt.callbackKind || 'Meeting callback',
                date: callbackDate,
                time: callbackTimeLabel,
                status: appt.callbackCompleted ? 'Completed' : (appt.callbackTriggered ? 'Warm Callback' : 'Pending'),
                primaryStatus: 'Pending',
                callbackSetting: undefined,
                callbackTriggered: false,
                callbackCompleted: Boolean(appt.callbackCompleted),
            });
        });
        return [...appointments, ...derived];
    }, [appointments]);

    // Shared appointment filter used by every calendar/list mode.
    const filteredAppointments = useMemo(() => {
        return activityAppointments.filter(appt => {
            const matchesStatus = statusFilter === 'all' || (statusFilter === 'Completed' ? ['Completed', 'Held'].includes(appt.status) : statusFilter === 'Held' ? ['Held', 'Completed'].includes(appt.status) : statusFilter === 'Canceled' ? ['Canceled', 'Cancelled'].includes(appt.status) : appt.status === statusFilter);
    ...
    }, [activityAppointments, statusFilter, assignedFilter, tagFilter, activityTypeFilter, timezoneFilter, searchTerm, getActivityKind]);
    ...
    const listFilteredAppointments = useMemo(() => {
        ...
        return filteredAppointments.filter((appt) => {
            ...
            const completed = ['Completed', 'Held', 'Canceled', 'Cancelled', 'No Show'].includes(appt.status || '') || Utils.isNoShow(appt) || (getActivityKind(appt) === 'callback' && appt.callbackCompleted === true);
            const overdue = Boolean(apptDate && apptDate < today && !completed);
            if (getActivityKind(appt) === 'callback' && appt.callbackCompleted) return false;
            ...
            if (getActivityKind(appt) === 'meeting') {
                if (meetingStageFilter === 'initial' && String(appt.followUpType || '').toLowerCase().includes('follow')) return false;
                if (meetingStageFilter === 'followup' && !String(appt.followUpType || '').toLowerCase().includes('follow')) return false;
                if (meetingStatusFilters.length && !(meetingStatusFilters.includes(appt.status || '') || (meetingStatusFilters.includes('Completed') && appt.status === 'Held') || (meetingStatusFilters.includes('Held') && appt.status === 'Completed') || (meetingStatusFilters.includes('Cancelled') && appt.status === 'Canceled'))) return false;
            }
    ...
    }, [filteredAppointments, viewMode, listPreset, activityTypeFilter, activitySubtypeFilter, includeCompleted, customStart, customEnd, todayStr, getActivityKind, meetingStageFilter, meetingStatusFilters]);
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
                        ...
                        {([['meeting', 'Meetings', '#8b9cff'], ['callback', 'Callbacks', '#fbbf24'], ['followup', 'Follow-ups', '#34d399']] as const).map(([value, label, dot]) => (
                            <div key={value} style={{ position: 'relative' }}>
                                <button key={value} onClick={() => { setActivityTypeFilter(activityTypeFilter === value ? 'all' : value); setActivitySubtypeFilter('all'); setOpenActivityMenu(openActivityMenu === value ? null : value); }} aria-pressed={activityTypeFilter === value} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '7px 11px', borderRadius: '8px', border: `1px solid ${activityTypeFilter === value ? dot : '#2a3852'}`, background: activityTypeFilter === value ? '#162036' : '#0d1527', color: '#e2e8f0', fontSize: '11px', fontWeight: 800, cursor: 'pointer' }}><span style={{ width: '6px', height: '6px', borderRadius: '50%', background: dot }}></span>{label}<i className={`fas fa-chevron-${openActivityMenu === value ? 'up' : 'down'}`} style={{ fontSize: '9px', opacity: .7 }} /></button>
                                {openActivityMenu === value && (
                                    <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 70, width: value === 'meeting' ? 270 : 235, padding: 12, border: '1px solid #334155', borderRadius: 12, background: '#151a25', boxShadow: '0 18px 40px rgba(0,0,0,.42)' }}>
                                        {value === 'meeting' ? <>
                                            <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800, marginBottom: 7 }}>Meeting</div>
                                            <div style={{ display: 'flex', border: '1px solid #334155', borderRadius: 9, overflow: 'hidden', marginBottom: 12 }}>
                                                {([['all','All'],['initial','Initial'],['followup','Follow-up']] as const).map(([stage, text]) => <button key={stage} onClick={() => { setMeetingStageFilter(stage); setOpenActivityMenu(null); }} style={{ flex: 1, border: 0, padding: '7px 8px', background: meetingStageFilter === stage ? '#374151' : 'transparent', color: '#e5e7eb', fontSize: 11, cursor: 'pointer' }}>{text}</button>)}
                                            </div>
                                            <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800, marginBottom: 7 }}>Status</div>
                                            {['Meeting Booked','Rescheduled','Completed','No Show','Canceled','Quarantined'].map(status => <label key={status} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', color: '#d1d5db', fontSize: 11, cursor: 'pointer' }}><input type="checkbox" checked={meetingStatusFilters.includes(status) || ((status === 'Completed' || status === 'Held') && (meetingStatusFilters.includes('Completed') || meetingStatusFilters.includes('Held')))} onChange={() => setMeetingStatusFilters(prev => prev.includes(status) ? prev.filter(x => x !== status) : [...prev, status])} />{status === 'Canceled' ? 'Cancelled' : status}</label>)}
                                        </> : <>
                                            <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800, marginBottom: 7 }}>{value === 'callback' ? 'Callbacks' : 'Follow-ups'}</div>
                                            <button onClick={() => { setActivitySubtypeFilter('all'); setOpenActivityMenu(null); }} style={{ width: '100%', textAlign: 'left', padding: '8px 9px', borderRadius: 7, border: '1px solid #334155', background: '#0d1527', color: '#e5e7eb', cursor: 'pointer', fontSize: 11 }}>All {value === 'callback' ? 'Callbacks' : 'Follow-ups'}</button>
                                            {Array.from(new Set(filteredAppointments.filter(a => getActivityKind(a) === value).map(a => value === 'callback' ? (a.callbackKind || 'Callback') : (a.followUpType || 'Follow-up')))).sort().map(sub => <button key={sub} onClick={() => { setActivitySubtypeFilter(sub); setOpenActivityMenu(null); }} style={{ width: '100%', textAlign: 'left', marginTop: 5, padding: '8px 9px', borderRadius: 7, border: '1px solid transparent', background: 'transparent', color: '#cbd5e1', cursor: 'pointer', fontSize: 11 }}>{sub}</button>)}
                                        </>}
                                    </div>
                                )}
                            </div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'auto' }}>
                                    ...
                                    <tbody>
                                        {sortedListAppointments.map(appt => {
                                            ...
                                            const openRecord = () => onSelectAppointment(appt.callbackSourceId ? (appointments.find(item => item.id === appt.callbackSourceId) || appt) : appt);
                                            const markDone = async () => {
                                                if (completed) return;
                                                try {
                                                    if (kind === 'callback' && appt.callbackSourceId) {
                                                        const source = appointments.find(item => item.id === appt.callbackSourceId);
                                                        if (source) await FirestoreService.saveAppointment({ ...source, callbackCompleted: true, callbackTriggered: true, updatedAt: new Date().toISOString() });
                                                    } else {
                                                        await FirestoreService.saveAppointment({ ...appt, status: 'Completed', primaryStatus: Utils.getPrimaryStatus('Completed'), updatedAt: new Date().toISOString() });
                                                    }
                                                } catch (error: any) { alert(error?.message || 'Unable to mark the activity done.'); }
                                            };
                                            const togglePause = async () => {
                                                try { await FirestoreService.saveAppointment({ ...(appt.callbackSourceId ? (appointments.find(item => item.id === appt.callbackSourceId) || appt) : appt), callbackPaused: !appt.callbackPaused, updatedAt: new Date().toISOString() }); }
