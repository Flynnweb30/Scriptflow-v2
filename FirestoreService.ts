...
export const FirestoreService = {
  ...
  async saveAppointment(appointment: Partial<Appointment> & { id: string }): Promise<void> {
    ...
    const nextTimezone = appointment.timezone ? normalizeUSTimezone(appointment.timezone) : existing?.timezone;
    const callbackConfigChanged = Boolean(existing && (
      existing.callbackSetting !== appointment.callbackSetting ||
      existing.date !== appointment.date ||
      existing.time !== appointment.time ||
      normalizeUSTimezone(existing.timezone) !== normalizeUSTimezone(nextTimezone)
    ));
    const callbackEnabled = appointment.callbackSetting && appointment.callbackSetting !== 'none';
    const callbackLifecycle = callbackEnabled
      ? (callbackConfigChanged || !existing ? { callbackTriggered: false, callbackCompleted: false } : {
          callbackTriggered: Boolean(appointment.callbackTriggered),
          callbackCompleted: Boolean(appointment.callbackCompleted),
        })
      : { callbackTriggered: false, callbackCompleted: false, callbackPaused: false };
    const data = {
      ...appointment,
